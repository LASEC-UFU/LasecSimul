...
export function snapToWireGrid(point, gridSize = WIRE_GRID_SIZE) {
    return {
        x: Math.round(point.x / gridSize) * gridSize,
        y: Math.round(point.y / gridSize) * gridSize,
    };
}
export function snapCoordinate(value, step) {
...
export function orthogonalSegmentPoints(fromPos, toPos) {
...
export function buildOrthogonalPath(anchors) {
    if (anchors.length === 0)
        return [];
