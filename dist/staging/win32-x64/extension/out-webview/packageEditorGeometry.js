 ...
 * viewBox visível. */
export function computeBounds(pkg) {
    let minX = 0;
    let minY = 0;
    let maxX = pkg.width;
    let maxY = pkg.height;
...
export function applyShapeOrigin(shape, next) {
...
export function inferPinPlacement(point, width, height) {
    const distances = [
...
export function nextFreePinId(existingIds) {
