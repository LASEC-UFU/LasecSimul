import { buildPaletteTree } from "./paletteTree.js";
const vscode = typeof acquireVsCodeApi === "function" ? acquireVsCodeApi() : undefined;
const app = document.getElementById("app");
const initialState = window.__LASECSIMUL_PALETTE_STATE__ ?? { catalog: [], language: "pt-BR" };
...
function collectVisibleComponents(nodes) {
    const items = [];
...
function render() {
    ...
    input.addEventListener("keydown", (event) => {
        if (event.key === "Enter" && visibleComponents.length === 1) {
            ...
            if (singleMatch)
                vscode?.postMessage({ type: "addComponent", typeId: singleMatch.typeId });
        }
    });
