...
exports.configurePackageEditor = configurePackageEditor;
exports.isPackageEditorActive = isPackageEditorActive;
exports.enterPackageEditor = enterPackageEditor;
exports.renderPackageEditorView = renderPackageEditorView;
exports.markPackageSaved = markPackageSaved;
exports.applyLoadedBackgroundImage = applyLoadedBackgroundImage;
/**
...
function configurePackageEditor(send, onExit, onChange) {
    sendMessage = send;
    onExitCallback = onExit;
    onChangeCallback = onChange;
}
function isPackageEditorActive() {
...
function enterPackageEditor(payload) {
    editorState = {
...
// ── Geometria compartilhada (mesma convenção do renderizador de leitura) ──────────────────────────
function pinTip(pin) {
    const rad = (pin.angle * Math.PI) / 180;
    return { x: pin.x + Math.cos(rad) * pin.length, y: pin.y + Math.sin(rad) * pin.length };
}
/** Mesmo cálculo de `componentSymbols.ts::resolvePackageLayout` -- canvas precisa da MESMA folga
 * (leads podem sair de `0..width`/`0..height`) pra nada ficar cortado/fora da viewBox. */
...
function renderCanvas(state) {
    ...
    bodyRect.addEventListener("click", (event) => {
        event.stopPropagation();
        if (state.addPinMode) {
            addPinAtPoint(state, { x: event.offsetXLocal ?? 0, y: 0 });
            return;
        }
...
function addPinAtPoint(state, point) {
    const w = state.pkg.width;
    const h = state.pkg.height;
    const distances = [
        { side: "top", distance: Math.abs(point.y) },
        { side: "bottom", distance: Math.abs(point.y - h) },
        { side: "left", distance: Math.abs(point.x) },
        { side: "right", distance: Math.abs(point.x - w) },
    ];
    distances.sort((a, b) => a.distance - b.distance);
    const side = distances[0]?.side ?? "right";
    const clampedX = Math.max(0, Math.min(w, point.x));
    const clampedY = Math.max(0, Math.min(h, point.y));
    let x = clampedX;
    let y = clampedY;
    let angle = 0;
    if (side === "top") {
    ...
    const existingIds = new Set(state.pkg.pins.map((pin) => pin.id));
    let suffix = state.pkg.pins.length + 1;
    let id = `pin${suffix}`;
    while (existingIds.has(id))
        id = `pin${++suffix}`;
    const pin = { id, x: Math.round(x), y: Math.round(y), angle, length: 8, label: id };
...
/** Move a forma TODA pelo delta entre a origem anterior e `next` -- linha precisa mover os dois
 * pontos juntos (não só `x1`/`y1`), senão "arrastar" uma linha a esticaria em vez de movê-la. */
...
function renderBodySidebar(state) {
    ...
    if (backgroundKind === "image" || backgroundKind === "svg") {
        ...
        section.appendChild(actionButton("Carregar imagem…", () => {
            sendMessage({ version: messages_js_1.WEBVIEW_MESSAGE_VERSION, type: "requestPickPackageBackgroundImage" });
        }));
    }
...
function savePackage(state) {
    sendMessage({
        version: messages_js_1.WEBVIEW_MESSAGE_VERSION,
...
function markPackageSaved() {
    if (editorState)
        editorState.dirty = false;
}
/** Chamado pelo handler de `"packageBackgroundImageLoaded"` em `main.ts`. */
function applyLoadedBackgroundImage(kind, data) {
    if (!editorState)
        return;
