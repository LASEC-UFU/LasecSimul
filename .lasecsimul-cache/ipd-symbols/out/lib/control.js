...
exports.controlHardware2 = exports.controlHardware = void 0;
const S = 1.5;
exports.controlHardware = [
    {
        id: 'ctl.interlock',
        name: 'Interlock',
        category: 'control',
        gridSize: { w: 3, h: 3 },
        render: () => `<polygon points="12,0 24,12 12,24 0,12" fill="none" stroke="currentColor" stroke-width="${S}"/>` +
...
const path5 = (d, w = 1.5) => `<path d="${d}" fill="none" stroke="currentColor" stroke-width="${w}" stroke-linejoin="round"/>`;
const text5 = (x, y, t, size = 8) => `<text x="${x}" y="${y}" font-size="${size}" font-family="sans-serif" text-anchor="middle" fill="currentColor" stroke="none">${t}</text>`;
function gate(id, name, label, extra = '') {
    return {
        id,
        name,
        category: 'control',
...
function systemBox(id, name, label, outerWidth = 1.5) {
    return {
        id,
        name,
        category: 'control',
        gridSize: { w: 5, h: 3 },
        render: () => path5('M0 0 h40 v24 h-40 Z', outerWidth) + path5('M4 4 h32 v16 h-32 Z') + text5(20, 15, label),
        ports: [
            { id: 'w', x: 0, y: 12, kind: 'signal' },
